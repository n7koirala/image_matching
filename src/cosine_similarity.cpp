...
double CosineSimilarity::plaintextInnerProduct(int dim, vector<double> x,
                                               vector<double> y) {
  double prod = 0.0;
  for (int i = 0; i < dim; i++) {
    prod += x[i] * y[i];
 ...
 * domain */
double CosineSimilarity::plaintextCosineSim(int dim, vector<double> x,
                                            vector<double> y) {
  return plaintextInnerProduct(dim, x, y) /
         (plaintextMagnitude(dim, x) * plaintextMagnitude(dim, y));
}

/* Append the vector source onto the end of the vector dest, n times */
void CosineSimilarity::concatenateVectors(vector<double> &dest,
...
Ciphertext<DCRTPoly>
CosineSimilarity::approxInverseMagnitude(int dim, CryptoContext<DCRTPoly> cc,
